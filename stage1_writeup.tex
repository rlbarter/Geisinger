...
blending for each dataset mentioned above as well as a general description of our indented analysis. Given that we have external data of various different levels of granularity, we will describe each situation individually.
...
where $S = cov({\bf x}_i^c, {\bf x}_j^e)$ is the covariance matrix, and the inverse correlation metric:
...
assess reliability: compare with if we used the obesity dataset. Comapre over subsetting and different imputation methods, etc.
